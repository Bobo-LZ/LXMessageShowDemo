...
+ (BOOL)createTable:(Class)cls uid:(NSString *)uid {
    ...
    NSString *createTableSql = [NSString stringWithFormat:@"create table if not exists %@(%@, primary key(%@))", tableName, [XWModelTool columnNamesAndTypesStr:cls], primaryKey];
    
    
    // 2. 执行
    return [XWSqliteTool deal:createTableSql uid:uid];
...
+ (BOOL)isTableRequiredUpdate:(Class)cls uid:(NSString *)uid {
    ...
    // 2. 获取当前表格, 所有字段名称, 并排序
    NSArray *tableNames = [XWTableTool tableSortedColumnNames:cls uid:uid];
...
+ (BOOL)updateTable:(Class)cls uid:(NSString *)uid {
    ...
    NSArray *oldNames = [XWTableTool tableSortedColumnNames:cls uid:uid];
    ...
    return [XWSqliteTool dealSqls:execSqls uid:uid];
...
+ (BOOL)saveOrUpdateModel:(id)model uid:(NSString *)uid {
    ...
    // 1. 判断表格是否存在, 不存在, 则创建
    if (![XWTableTool isTableExists:cls uid:uid]) {
    ...
    NSArray *result = [XWSqliteTool querySql:checkSql uid:uid];
    
    
    // 获取字段名称数组
    NSArray *columnNames = [XWModelTool classIvarNameTypeDic:cls].allKeys;
    ...
    return [XWSqliteTool deal:execSql uid:uid];
}
...
+ (BOOL)deleteModel:(id)model uid:(NSString *)uid {
    ...
    return [XWSqliteTool deal:deleteSql uid:uid];
...
+ (NSArray *)queryAllModels:(Class)cls uid:(NSString *)uid {
    ...
    // xx 字符串
    // oo 字符串
    NSArray <NSDictionary *>*results = [XWSqliteTool querySql:sql uid:uid];
...
+ (NSArray *)parseResults:(NSArray <NSDictionary *>*)results withClass:(Class)cls {
    ...
    // 属性名称 -> 类型 dic
    NSDictionary *nameTypeDic = [XWModelTool classIvarNameTypeDic:cls];
